...
#
#  运行逻辑：
#    - 当编译目标为 x86-64 时，此脚本不做任何改动，完全保留
#      shell/custom-packages.sh 中定义的全部软件。
#    - 当编译目标为小米 CR6608 时，先移除大量不适合嵌入式的
#      重型软件，再添加轻量级专属插件。
# =====================================================
...
if [[ "$TARGET_DEVICE" == "xiaomi_mi-router-cr6608" ]]; then
    ...
    # --- Samba4 文件共享（体积大，可用 ksmbd，但都已移除）---
    ...
    # --- Easytier 一个现代组网工具（若使用 zerotier 则多余）---
    ...
    # ③ 如果还想保留某个被移除的包，可以再加回来
    # 例如 OpenClash 默认已在原列表中，无需额外添加
    # 若需要其他保留项，在此处添加即可
fi
